...
class AudioBridge:
    ...
    def __init__(self, log_cb=None, config: dict = None):
        ...
        # ── Env var'lar öncelikli ──
        self._venv_python = os.environ.get(
            "VENV_AUDIO_PYTHON",
            cfg.get("venv_audio_python", self.DEFAULT_VENV_PYTHON)
        )
        self._worker_script = os.environ.get(
            "AUDIO_WORKER_SCRIPT",
            cfg.get("audio_worker_script", self.DEFAULT_WORKER_SCRIPT)
        )
